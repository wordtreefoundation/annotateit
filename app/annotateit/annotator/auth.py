...
class Authenticator(object):
    ...
    def _decode_request_token(self, request):
        ...
        if not key:
            logging.warn("consumerKey not found: " + key)
            return False

        consumer = self.consumer_fetcher(key)
        if not consumer:
            logging.warn("consumer could not be fetched: " + json.dumps(unsafe_token))
            return False

        try:
            logging.warn("decoding")
            return decode_token(token,
                                secret=consumer.secret,
                                ttl=consumer.ttl)
        except TokenInvalid, e:  # catch inauthentic or expired tokens
            logging.warn("decode failed: " + str(e))
            return False
